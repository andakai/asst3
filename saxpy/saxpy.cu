...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double executionStartTime = CycleTimer::currentSeconds();
    saxpy_kernel<<<blocks, threadsPerBlock>>>(N, alpha, device_x, device_y, device_result);
    cudaDeviceSynchronize();
    double executionEndTime = CycleTimer::currentSeconds();
    //
    ...
    double executionDuration = executionEndTime - executionStartTime;
    printf("Effective BW by CUDA saxpy: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * overallDuration, GBPerSec(totalBytes, overallDuration));
    printf("[only execution] Effective BW by CUDA saxpy: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * executionDuration, GBPerSec(totalBytes, executionDuration));
    //
